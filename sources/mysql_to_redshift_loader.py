...
#Copyright (c): 2016 Alex Buzunov. Free to use, modify, or distribute (star this repo).
...
#----------------------------------------------------------------------
  ...
  set MYSQL_CLIENT_HOME="C:\Temp\mysql\bin"
...
MYSQL_CLIENT_HOME = os.getenv('MYSQL_CLIENT_HOME') 
assert MYSQL_CLIENT_HOME, 'MYSQL_CLIENT_HOME is not set'
import imp
...
def sendStreamGz(bucket, s3_key, pipe, suffix='.gz'):
	...
	q_file =os.path.splitext(os.path.basename(opt.mysql_query_file))[0]
...
if __name__ == "__main__":		
	...
	parser.add_option("-q", "--mysql_query_file", dest="mysql_query_file", type=str)
	parser.add_option("-d", "--mysql_col_delim", dest="mysql_col_delim", type=str, default='|')
	parser.add_option("-a", "--mysql_quote", dest="mysql_quote", default='')	
	parser.add_option("-j", "--mysql_user", dest="mysql_user", default='alex')	
	parser.add_option("-1", "--mysql_pwd", dest="mysql_pwd", default='mysql_pwd')	
	parser.add_option("-f", "--mysql_db_name", dest="mysql_db_name", default='test')	
	parser.add_option("-n", "--mysql_db_server", dest="mysql_db_server", default='127.0.0.1')	

	#parser.add_option("-e", "--mysql_add_header", dest="ora_add_header",  action="store_true", default=False)
	parser.add_option("-l", "--mysql_lame_duck", dest="mysql_lame_duck", type=int, default=0)
	...
	assert os.path.isfile(opt.mysql_query_file), 'Query file "%s"\ndoes not exists.' % opt.mysql_query_file
	...
	assert RepresentsInt(opt.mysql_lame_duck), '[-l] --mysql_lame_duck is not of type "integer".'
	...
	sys.stdout.write('Elapsed: MySQL+S3    :%s sec.\n' % round((time.time() - start_time),2))
